...
export function ProjectChat({ projectId, tenderId }) { // הרכיב שאחראי על הצ'אט החכם עם ברבור
  // --- המשתנים של הצ'אט (מה המצב כרגע?) ---
  const [messages, setMessages] = useState([ // רשימת ההודעות שרואים על המסך
    { id: 1, type: 'bot', text: 'אהלן, ברבור כאן 🦢. אני שולט בכל המסמכים והנתונים של הפרויקט או המכרז. מה נרצה לבדוק היום? אני יכול לעבור על הצעות מחיר, אומדנים או כל מה שקבור באותיות הקטנות.' }
  ]);
  ...
  // פונקציה שמביאה את רשימת הקבצים שברבור כבר מכיר בפרויקט/מכרז הזה
  const fetchFiles = async () => {
    try {
      const url = projectId 
        ? `${api.baseUrl}/projects/${projectId}/files`
        : `${api.baseUrl}/tenders/${tenderId}/files`;
  ...
  // מביאים את הקבצים ברגע שנכנסים לפרויקט או למכרז
  useEffect(() => {
    if (projectId || tenderId) fetchFiles();
  }, [projectId, tenderId]);
  ...
  const handleSend = async (e) => {
    ...
    try {
      const url = projectId
        ? `${api.baseUrl}/projects/${projectId}/chat`
        : `${api.baseUrl}/tenders/${tenderId}/chat`;
      const response = await fetch(url, {
        method: 'POST',
      ...
      <div className="p-4 border-t border-border bg-surface">
        <form onSubmit={handleSend} className="flex gap-2">
          ...
          <input 
            ...
            placeholder={tenderId ? "שאל את ברבור משהו על המכרז (לדוגמה: כמה מטר ריצוף צריך?)..." : "שאל את ברבור משהו על הפרויקט..."} 
