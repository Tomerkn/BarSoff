...
export const askQuestion = async (projectId, question, includeGlobal = true) => {
  try {
    ...
    const promptParts = [
      ...
      בסוף התשובה, עליך לציין את רמת הוודאות שלך במידע שמצאת בפורמט: [CONFIDENCE]ציון בין 0 ל-100[/CONFIDENCE].
...
export const analyzeTender = async (filePath) => {
  try {
    ...
    const prompt = `
      ...
      בסוף הניתוח, עליך לציין את רמת הוודאות שלך בניתוח בפורמט: [CONFIDENCE]ציון בין 0 ל-100[/CONFIDENCE].
