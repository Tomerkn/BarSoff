...
function seed() { // פונקציה שממלאת נתונים ראשוניים במערכת כדי שלא תהיה ריקה
  ...
  const bud1 = insertBudget.run(proj1.lastInsertRowid, "עבודות שלד", 1000000);
  const bud2 = insertBudget.run(proj1.lastInsertRowid, "חשמל ותקשורת", 500000);
  const bud3 = insertBudget.run(proj1.lastInsertRowid, "אינסטלציה", 300000);
