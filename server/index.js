...
app.post('/api/tenders', upload.single('file'), async (req, res) => { // מעלה מכרז חדש ומתחיל לנתח אותו
  ...
  const info = insert.run(req.file.originalname, req.file.filename, new Date().toISOString(), 'מנתח...');
  
  // הניתוח רץ ברקע כדי שלא תצטרך לחכות מול מסך טוען
  analyzeTender(req.file.path).then(analysis => {
    db.prepare('UPDATE tenders SET analysis = ?, status = ? WHERE id = ?').run(analysis, 'נותח', info.lastInsertRowid);
    ingestDocument('global', req.file.path); // לומד את המכרז לזיכרון לטווח ארוך
  }).catch(console.error);
...
app.post('/api/tenders/:id/proposal', async (req, res) => { // פקודה לברבור: "תכין לי הצעת מחיר"
  ...
  const proposal = await generateProposal(path.join(UPLOADS_DIR, tender.filename));
